...
- **不能装原生二进制**：glibc / linux-x64 预编译包（`npm rebuild`、C/C++ 扩展）无法运行；扩展只限纯 JS 包或自带工具链
- **无 Python / gcc**：脚本任务用 node / bash 替代；编译型项目无法在设备上构建
- **不能装系统包**：Alpine proot 活环境在路线图（M3）中，当前不能 `apt/apk add`
